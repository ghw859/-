...
## 五、下午追加：任务 4（越权/隔离）+ 任务 6（失败路径）结果

### 任务 4 越权/隔离 —— 全过 ✅

| 用例 | 结果 |
|------|------|
| user2 查自己的审计链 | ✅ 0 条，看不到 user1 的链（operator_id 隔离生效） |
| user2 访问管理员审计接口 /audit/logs、/verify | ✅ 10003 拒绝 |
| user2 读/推进 user1 的预约 41 | ✅ 10003 拒绝 |
| user2 查自己的预约列表 | ✅ 0 条，不串数据 |
| user2 凭 T 号查 user1 的直通码 | ⚠️ code=0 可查（**设计如此**：凭证号即凭据、图片接口本就公开放行；返回无 PII，仅号/时间/类型）。知悉即可，非漏洞 |
| 测试账号 e2e_user2 | 已删除，users 表恢复 1 人 |

### 任务 6 失败路径 —— 全过 ✅

| 用例 | 结果 |
|------|------|
| 已取消预约推进（id=35） | ✅ 20006「预约已取消，无法推进进度」 |
| 已完成的预约取消（id=40） | ✅ 20005「预约已办理完成，无法取消」 |
| 已失效预约删除（id=36） | ✅ 20004「预约已过期，无法取消」 |
| 同时段重复预约 | ✅ 20001「该时段已在其他网点预约」 |
| 伪造 token | ✅ 11008「Token无效」 |
| 预检缺材料（走查第 9 步） | ✅ passed=false + 中文缺失清单，不跳错误码 |
